 ...
 * @version $Id: sitemapxml.php, v 3.2.12 19.09.2013 8:06:18 AndrewBerezin $
 */
...
// generate sitemap index file
  function GenerateSitemapIndex() {
    ...
    if ($this->genxml) {
      ...
      echo sprintf(TEXT_WRITTEN, $this->sitemapFileItems++, $this->sitemapFileSizeTotal, filesize($this->savepathIndex . $this->sitemapFileName)) . '<br /><br />';
...
//    $this->sitemapFileName = null;
  ...
  function _SitemapCloseFile() {
    ...
    $this->_fileClose();
    echo sprintf(TEXT_FILE_SITEMAP_INFO, $this->base_url . $this->sitemapFileName, $this->base_url . $this->sitemapFileName, $this->sitemapFileItems, $this->sitemapFileSize, filesize($this->savepath . $this->sitemapFileName)) . '<br />';
  }
  ...
  function _checkDublicateLoc($loc) {
    ...
    if ($this->checkDublicates == 'true') {
      ...
      $url_hash = md5($loc);
      $sql = "SELECT COUNT(*) AS total FROM " . TABLE_SITEMAPXML_TEMP . " WHERE url_hash=:urlHash";
      $sql = $db->bindVars($sql, ':urlHash', $url_hash, 'string');
      $check = $db->Execute($sql);
      if ($check->fields['total'] > 0) return false;
...
///////////////////////
  function _fileOpen($filename, $path='') {
    if ($path == '') {
      $path = $this->savepath;
    }
    $this->fn = $filename;
