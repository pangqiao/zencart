...
class plugin {
	...
	/**
	 * Retrieves the description of this plugin. The description is saved in the
	 * database along with the configuration group. Currently it is not shown in
	 * the Zen Cart admin.
	 *
	...
	/**
	 * Handles any changes to files required by this plugin.
	 * This is typically used to verify required modifications to core Zen Cart
	 * files have been made or to actually make changes to core Zen Cart files.
	 *
	 * Subclasses can optionally override this method.
	 * @param boolean $install true if installing / upgrading, false otherwise
	 * @return boolean true if the changes succeed, false otherwise.
	 */
	public function handleFileChanges($install = true) { return true; }
	/**
	 * Handles the installation and removal of any new admin menus required
	 * by this plugin.
	 *
	 * Subclasses can optionally override this method.
	 * @param boolean $install true if installing / upgrading, false otherwise
	 * @return boolean true if the changes succeed, false otherwise.
	 */
	public function handleAdminMenuChanges($install = true) { return true; }
	 ...
	 */
	protected function getAutoLoaderFiles() {
		return array(
			'class' => DIR_FS_ADMIN . DIR_WS_CLASSES .
				strtolower($this->getUniqueKey()) . '_plugin.php',
			'loader' => DIR_FS_ADMIN . DIR_WS_INCLUDES . 'auto_loaders/config.' .
	...
	protected function verifyNewFiles() {
		...
		// Verify installer file(s)
		foreach($this->getAutoLoaderFiles() as $key => $file) {
			if(!file_exists($file)) {
				$messageStack->add(
					sprintf(
	...
	protected function removeConfiguration() {
		...
		foreach($this->getDefaultConfiguration() as $key => $data) {
			$key = $this->getUniqueKey() . '_' . $key;
			if(!removeConfigurationOption($key)) {
				$messageStack->add(
					sprintf(
						PLUGIN_REMOVE_ERROR_DATABASE_KEY,
						$this->getUniqueName(),
	...
	/**
	 * Creates a generic init includes file to either install or remove this
	 * plugin. Does nothing and returns true if the file already exists.
	 *
	 ...
	 */
	protected function createInitIncludeFile($install = true) {
		global $messageStack;

		$files = $this->getAutoLoaderFiles();
		$type = ($install ? 'install' : 'remove');
		if(file_exists($files[$type])) return true;
		...
		if(@file_put_contents($files[$type], $contents) === false) {
			$messageStack->add(
				sprintf(
					PLUGIN_INSTALL_ERROR_FILE_NOT_FOUND,
					$this->getUniqueName(),
	...
	/**
	 * Creates a generic onetime auto load file to either install or remove this
	 * plugin. Does nothing and returns true if the file already exists.
	 *
	 * @param boolean $install true to create the onetime auto load install file,
	 *      false to create the onetime auto load removal file.
	 * @return boolean true if the file was created successfully, false otherwise.
	 */
	protected function createOnetimeFile($install = true) {
		global $messageStack;

		$files = $this->getAutoLoaderFiles();
		$type = ($install ? 'install' : 'remove');
		if(file_exists($files['loader'])) return true;
		...
		if(@file_put_contents($files['loader'], $contents) === false) {
			$messageStack->add(
				sprintf(
					PLUGIN_INSTALL_ERROR_FILE_NOT_FOUND,
					$this->getUniqueName(),
