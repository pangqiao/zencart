 ...
 */
  ...
  if (zen_not_null($action)) {
    ...
    case 'change_quantity_confirm':
        $quantity = zen_db_prepare_input($_POST['quantity']);
        $quantity = (int)$quantity;
        if (is_int($quantity)) { 
            $sql = "UPDATE " . TABLE_PRODUCTS . " SET products_quantity = '" . $quantity . "' WHERE products_id ='" . $_POST['products_id']. "'";
            $db->Execute($sql);
            if ($quantity == 0) { 
              if (SHOW_PRODUCTS_SOLD_OUT == '0') {
                $db->Execute("update " . TABLE_PRODUCTS . " set products_status = 0 where products_id = '" . $_POST['products_id'] . "'");
              }
            } else { 
                $db->Execute("update " . TABLE_PRODUCTS . " set products_status = 1 where products_id = '" . $_POST['products_id'] . "'");
            }
        }
        zen_redirect(zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $_GET['products_id'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')));
        break; 
    // eof That Software Guy Quick Quantity Update 
...
<?php
  ...
  switch ($action) {
    ...
    */
    ...
    $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_CHANGE_QUANTITY. '</b>');

    $contents = array('form' => zen_draw_form('products', FILENAME_CATEGORIES, 'action=change_quantity_confirm&cPath=' . $cPath . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')) . zen_draw_hidden_field('products_id', $pInfo->products_id));
    $contents[] = array('text' => sprintf(TEXT_CHANGE_QUANTITY_INTRO, $pInfo->products_name));
    $contents[] = array('align' => 'center', 'text' => '<br />' . '<br />' . zen_draw_input_field('quantity', '', 'size="6"'));
    $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
    break;
    // eof That Software Guy Quick Quantity Update
    case 'copy_to':
